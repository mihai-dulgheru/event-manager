...
public final class Database {
    ...
    public static void populate() {
        try {
            ...
            Contract[] contracts = getContracts(clients);
            for (Contract contract : contracts) {
                contract.insert();
            }

            Locatie[] locations = getLocations();
            for (Locatie locatie : locations) {
                locatie.insert();
            }

            Eveniment[] events = getEvents(contracts, locations);
            for (Eveniment eveniment : events) {
                eveniment.insert();
            }

            Serviciu[] services = getServices();
            for (Serviciu serviciu : services) {
                serviciu.insert();
            }

            Pachet[] packets = getPackets(events, services);
            ...
            System.out.println("Database populated unsuccessfully!");
            Database.rollback();
        } finally {
            Database.disconnect();
        }
    ...
    public static Contract[] getContracts(Client[] clients) {
        return new Contract[]{
    ...
    private static Locatie[] getLocations() {
        return new Locatie[]{
    ...
    private static Eveniment[] getEvents(Contract[] contracts, Locatie[] locations) {
        return new Eveniment[]{
                new Eveniment(contracts[0].getId(), locations[0].getId(), TipEveniment.BOTEZ, "2023-12-17", 80, "Disney"),
                new Eveniment(contracts[1].getId(), locations[1].getId(), TipEveniment.CONCERT, "2023-12-18", 2000, "Retro"),
                new Eveniment(contracts[2].getId(), locations[2].getId(), TipEveniment.EXPOZITIE, "2023-12-19", 130, "Vintage"),
                new Eveniment(contracts[3].getId(), locations[3].getId(), TipEveniment.FESTIVAL, "2023-12-20", 4000, null),
                new Eveniment(contracts[4].getId(), locations[4].getId(), TipEveniment.NUNTA, "2023-12-21", 200, null),};
    }

    private static Serviciu[] getServices() {
        return new Serviciu[]{
